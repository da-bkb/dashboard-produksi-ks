...
st.markdown(f"### 🌴 RJP terhadap Budget (Jjg/Pkk)")
...
for col in ['Pokok', 'Jjg Akt.', 'Jjg Bgt.']:
...
with col_t1:
    ...
    df_total_mtd = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': df_mtd['Jjg Akt.'].sum() / pokok_kebun_mtd.sum(), 'Budget': df_mtd['Jjg Bgt.'].sum() / pokok_kebun_mtd.sum(), 'Var': (df_mtd['Jjg Akt.'].sum() - df_mtd['Jjg Bgt.'].sum()) / pokok_kebun_mtd.sum(), 'Pct': (df_mtd['Jjg Akt.'].sum() / df_mtd['Jjg Bgt.'].sum() * 100) - 100}])
    df_final_mtd = pd.concat([df_t_mtd, df_total_mtd], ignore_index=True)
    df_final_mtd.insert(0, 'No', range(1, len(df_final_mtd) + 1))
    df_final_mtd.columns = ['No', 'Kebun', 'Aktual (Jjg/Pkk)', 'Budget (Jjg/Pkk)', 'Gap', 'Var (%)']
    st.dataframe(df_final_mtd.style.format({'Aktual (Jjg/Pkk)': '{:,.2f}', 'Budget (Jjg/Pkk)': '{:,.2f}', 'Gap': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap']).map(style_budget_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_rjp_bgt_kebun_mtd")
...
with col_t2:
    ...
    df_total_ytd = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': df_ytd['Jjg Akt.'].sum() / pokok_kebun_ytd.sum(), 'Budget': df_ytd['Jjg Bgt.'].sum() / pokok_kebun_ytd.sum(), 'Var': (df_ytd['Jjg Akt.'].sum() - df_ytd['Jjg Bgt.'].sum()) / pokok_kebun_ytd.sum(), 'Pct': (df_ytd['Jjg Akt.'].sum() / df_ytd['Jjg Bgt.'].sum() * 100) - 100}])
    df_final_ytd = pd.concat([df_t_ytd, df_total_ytd], ignore_index=True)
    df_final_ytd.insert(0, 'No', range(1, len(df_final_ytd) + 1))
    df_final_ytd.columns = ['No', 'Kebun', 'Aktual (Jjg/Pkk)', 'Budget (Jjg/Pkk)', 'Gap', 'Var (%)']
    st.dataframe(df_final_ytd.style.format({'Aktual (Jjg/Pkk)': '{:,.2f}', 'Budget (Jjg/Pkk)': '{:,.2f}', 'Gap': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap']).map(style_budget_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_rjp_bgt_kebun_ytd")

# --- AFDELING ---
st.markdown("---")
st.markdown("### 🔎 Detail per Afdeling")
list_kebun = sorted(df_raw['Kebun'].dropna().unique())
kebun_terpilih = st.selectbox("Pilih Kebun untuk melihat detail Afdeling:", list_kebun, key="sb_rjp_bgt_afd")
df_m_afd = df_mtd[df_mtd['Kebun'] == kebun_terpilih].copy()
df_y_afd = df_ytd[df_ytd['Kebun'] == kebun_terpilih].copy()

if not df_m_afd.empty:
    df_a_mtd = df_m_afd.groupby('Afdeling').agg({'Jjg Akt.': 'sum', 'Jjg Bgt.': 'sum'}).reset_index()
    ...
    df_a_mtd['Target'] = df_a_mtd['Jjg Bgt.'] / df_a_mtd['Pokok']
    df_a_mtd['Pct'] = (df_a_mtd['Aktual'] / df_a_mtd['Target'] * 100).fillna(0)

    df_a_ytd = df_y_afd.groupby('Afdeling').agg({'Jjg Akt.': 'sum', 'Jjg Bgt.': 'sum'}).reset_index()
    ...
    df_a_ytd['Target'] = df_a_ytd['Jjg Bgt.'] / df_a_ytd['Pokok']
    ...
    with col_ta1:
        ...
        df_ta_mtd.columns = ['No', 'Afdeling', 'Aktual (Jjg/Pkk)', 'Budget (Jjg/Pkk)', 'Gap', 'Var (%)']
        st.dataframe(df_ta_mtd.style.format({'Aktual (Jjg/Pkk)': '{:,.2f}', 'Budget (Jjg/Pkk)': '{:,.2f}', 'Gap': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap']).map(style_budget_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_rjp_bgt_afd_mtd")
    ...
    with col_ta2:
        ...
        df_ta_ytd.columns = ['No', 'Afdeling', 'Aktual (Jjg/Pkk)', 'Budget (Jjg/Pkk)', 'Gap', 'Var (%)']
        st.dataframe(df_ta_ytd.style.format({'Aktual (Jjg/Pkk)': '{:,.2f}', 'Budget (Jjg/Pkk)': '{:,.2f}', 'Gap': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap']).map(style_budget_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_rjp_bgt_afd_ytd")