...
st.markdown(f"### 🎯 BJR terhadap Sensus (Kg/Jjg)")
...
for col in ['Kg Akt.', 'Jjg Akt.', 'BJR Sns.']:
...
df_k_mtd = df_mtd.groupby('Kebun').apply(
    lambda x: pd.Series({
        ...
        'Target': (x['BJR Sns.'] * x['Jjg Akt.']).sum() / x['Jjg Akt.'].sum() if x['Jjg Akt.'].sum() > 0 else 0
    })
).reset_index()
...
with col_t1:
    ...
    site_sns_mtd = (df_mtd['BJR Sns.'] * df_mtd['Jjg Akt.']).sum() / tot_jjg_mtd if tot_jjg_mtd > 0 else 0
    df_total_mtd = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': site_akt_mtd, 'Sensus': site_sns_mtd, 'Var': site_akt_mtd - site_sns_mtd, 'Pct': (site_akt_mtd / site_sns_mtd * 100) - 100 if site_sns_mtd > 0 else 0}])
...
with col_t2:
    ...
    site_sns_ytd = (df_ytd['BJR Sns.'] * df_ytd['Jjg Akt.']).sum() / tot_jjg_ytd if tot_jjg_ytd > 0 else 0
    df_total_ytd = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': site_akt_ytd, 'Sensus': site_sns_ytd, 'Var': site_akt_ytd - site_sns_ytd, 'Pct': (site_akt_ytd / site_sns_ytd * 100) - 100 if site_sns_ytd > 0 else 0}])
...
if not df_m_afd.empty:
    df_a_mtd = df_m_afd.groupby('Afdeling').agg({'Kg Akt.': 'sum', 'Jjg Akt.': 'sum', 'BJR Sns.': 'mean'}).reset_index()
    df_a_mtd['Aktual'] = df_a_mtd['Kg Akt.'] / df_a_mtd['Jjg Akt.']
    df_a_mtd.rename(columns={'BJR Sns.': 'Target'}, inplace=True)
    df_a_mtd['Pct'] = (df_a_mtd['Aktual'] / df_a_mtd['Target'] * 100).fillna(0)

    df_a_ytd = df_y_afd.groupby('Afdeling').agg({'Kg Akt.': 'sum', 'Jjg Akt.': 'sum', 'BJR Sns.': 'mean'}).reset_index()
    df_a_ytd['Aktual'] = df_a_ytd['Kg Akt.'] / df_a_ytd['Jjg Akt.']
    df_a_ytd.rename(columns={'BJR Sns.': 'Target'}, inplace=True)
    ...
    with col_ta1:
        ...
        df_ta_mtd['Gap'] = df_ta_mtd['Aktual'] - df_ta_mtd['Sensus']
        df_ta_mtd['Var (%)'] = (df_ta_mtd['Aktual'] / df_ta_mtd['Sensus'] * 100) - 100
        df_ta_mtd.insert(0, 'No', range(1, len(df_ta_mtd) + 1))
        st.dataframe(df_ta_mtd.style.format({'Aktual': '{:,.2f}', 'Sensus': '{:,.2f}', 'Gap': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap']).map(style_bjr_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_bjr_sns_afd_mtd")

    with col_ta2:
        df_ta_ytd = df_a_ytd[['Afdeling', 'Aktual', 'Target']].copy()
        df_ta_ytd.columns = ['Afdeling', 'Aktual', 'Sensus']
        df_ta_ytd['Gap'] = df_ta_ytd['Aktual'] - df_ta_ytd['Sensus']
        df_ta_ytd['Var (%)'] = (df_ta_ytd['Aktual'] / df_ta_ytd['Sensus'] * 100) - 100
        df_ta_ytd.insert(0, 'No', range(1, len(df_ta_ytd) + 1))
        st.dataframe(df_ta_ytd.style.format({'Aktual': '{:,.2f}', 'Sensus': '{:,.2f}', 'Gap': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap']).map(style_bjr_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_bjr_sns_afd_ytd")