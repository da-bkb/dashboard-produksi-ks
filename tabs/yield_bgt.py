...
with col_t1:
    ...
    df_total_mtd = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': df_mtd['Kg Akt.'].sum() / luas_kebun_mtd.sum() / 1000, 'Budget': df_mtd['Kg Bgt.'].sum() / luas_kebun_mtd.sum() / 1000, 'Var': (df_mtd['Kg Akt.'].sum() - df_mtd['Kg Bgt.'].sum()) / luas_kebun_mtd.sum() / 1000, 'Pct': (df_mtd['Kg Akt.'].sum() / df_mtd['Kg Bgt.'].sum() * 100) - 100}])
    ...
    st.dataframe(df_final_mtd.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Budget (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_budget_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_yield_bgt_kebun_mtd")
...
with col_t2:
    ...
    df_total_ytd = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': df_ytd['Kg Akt.'].sum() / luas_kebun_ytd.sum() / 1000, 'Budget': df_ytd['Kg Bgt.'].sum() / luas_kebun_ytd.sum() / 1000, 'Var': (df_ytd['Kg Akt.'].sum() - df_ytd['Kg Bgt.'].sum()) / luas_kebun_ytd.sum() / 1000, 'Pct': (df_ytd['Kg Akt.'].sum() / df_ytd['Kg Bgt.'].sum() * 100) - 100}])
    ...
    st.dataframe(df_final_ytd.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Budget (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_budget_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_yield_bgt_kebun_ytd")
...
if not df_m_afd.empty:
    ...
    with col_ta1:
        ...
        st.dataframe(df_ta_mtd.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Budget (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_budget_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_yield_bgt_afd_mtd")
    ...
    with col_ta2:
        ...
        st.dataframe(df_ta_ytd.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Budget (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_budget_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_yield_bgt_afd_ytd")