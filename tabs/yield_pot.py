...
st.markdown(f"### 🌟 Yield terhadap Potensi (Ton/Ha)")
...
with col_g1:
    ...
    fig_mtd.add_trace(go.Scatter(x=df_k_mtd["Kebun"], y=[None]*len(df_k_mtd), mode='lines', line=dict(color='#00B050', width=4), name='Potensi'))
    ...
    st.plotly_chart(fig_mtd, use_container_width=True, key="chart_yield_pot_kebun_mtd")
...
with col_g2:
    ...
    fig_ytd.add_trace(go.Scatter(x=df_k_ytd["Kebun"], y=[None]*len(df_k_ytd), mode='lines', line=dict(color='#00B050', width=4), name='Potensi'))
    ...
    st.plotly_chart(fig_ytd, use_container_width=True, key="chart_yield_pot_kebun_ytd")

def style_gap_black(val): return 'color: black; font-weight: bold;'
def style_pot_var_fill(val):
...
with col_t1:
    ...
    df_t_mtd.columns = ['Kebun', 'Aktual', 'Potensi']
    df_t_mtd['Var'] = df_t_mtd['Aktual'] - df_t_mtd['Potensi']
    df_t_mtd['Pct'] = (df_t_mtd['Aktual'] / df_t_mtd['Potensi'] * 100) - 100
    df_total_mtd = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': df_mtd['Kg Akt.'].sum() / luas_kebun_mtd.sum() / 1000, 'Potensi': df_mtd['Kg Pot.'].sum() / luas_kebun_mtd.sum() / 1000, 'Var': (df_mtd['Kg Akt.'].sum() - df_mtd['Kg Pot.'].sum()) / luas_kebun_mtd.sum() / 1000, 'Pct': (df_mtd['Kg Akt.'].sum() / df_mtd['Kg Pot.'].sum() * 100) - 100}])
    df_final_mtd = pd.concat([df_t_mtd, df_total_mtd], ignore_index=True)
    df_final_mtd.insert(0, 'No', range(1, len(df_final_mtd) + 1))
    df_final_mtd.columns = ['No', 'Kebun', 'Aktual (Ton/Ha)', 'Potensi (Ton/Ha)', 'Gap (Ton/Ha)', 'Var (%)']
    st.dataframe(df_final_mtd.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Potensi (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_pot_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_yield_pot_kebun_mtd")

with col_t2:
    st.markdown(f"##### 📋 Data Yield Per Kebun - s.d {pilihan_bulan}")
    df_t_ytd = df_k_ytd[['Kebun', 'Aktual', 'Target']].copy()
    df_t_ytd.columns = ['Kebun', 'Aktual', 'Potensi']
    df_t_ytd['Var'] = df_t_ytd['Aktual'] - df_t_ytd['Potensi']
    df_t_ytd['Pct'] = (df_t_ytd['Aktual'] / df_t_ytd['Potensi'] * 100) - 100
    df_total_ytd = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': df_ytd['Kg Akt.'].sum() / luas_kebun_ytd.sum() / 1000, 'Potensi': df_ytd['Kg Pot.'].sum() / luas_kebun_ytd.sum() / 1000, 'Var': (df_ytd['Kg Akt.'].sum() - df_ytd['Kg Pot.'].sum()) / luas_kebun_ytd.sum() / 1000, 'Pct': (df_ytd['Kg Akt.'].sum() / df_ytd['Kg Pot.'].sum() * 100) - 100}])
    df_final_ytd = pd.concat([df_t_ytd, df_total_ytd], ignore_index=True)
    df_final_ytd.insert(0, 'No', range(1, len(df_final_ytd) + 1))
    df_final_ytd.columns = ['No', 'Kebun', 'Aktual (Ton/Ha)', 'Potensi (Ton/Ha)', 'Gap (Ton/Ha)', 'Var (%)']
    st.dataframe(df_final_ytd.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Potensi (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_pot_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_yield_pot_kebun_ytd")

# --- AFDELING ---
st.markdown("---")
st.markdown("### 🔎 Detail per Afdeling")
list_kebun = sorted(df_raw['Kebun'].dropna().unique())
kebun_terpilih = st.selectbox("Pilih Kebun untuk melihat detail Afdeling:", list_kebun, key="sb_pot_afd")
df_m_afd = df_mtd[df_mtd['Kebun'] == kebun_terpilih].copy()
df_y_afd = df_ytd[df_ytd['Kebun'] == kebun_terpilih].copy()

if not df_m_afd.empty:
    df_a_mtd = df_m_afd.groupby('Afdeling').agg({'Kg Akt.': 'sum', 'Kg Pot.': 'sum'}).reset_index()
    ...
    df_a_mtd['Target'] = df_a_mtd['Kg Pot.'] / df_a_mtd['Luas'] / 1000
    df_a_mtd['Pct'] = (df_a_mtd['Aktual'] / df_a_mtd['Target'] * 100).fillna(0)

    df_a_ytd = df_y_afd.groupby('Afdeling').agg({'Kg Akt.': 'sum', 'Kg Pot.': 'sum'}).reset_index()
    ...
    df_a_ytd['Target'] = df_a_ytd['Kg Pot.'] / df_a_ytd['Luas'] / 1000
    ...
    with col_ga1:
        ...
        fig_amtd.add_trace(go.Scatter(x=df_a_mtd["Afdeling"], y=[None]*len(df_a_mtd), mode='lines', line=dict(color='#00B050', width=4), name='Potensi'))
        ...
        st.plotly_chart(fig_amtd, use_container_width=True, key="chart_yield_pot_afd_mtd")
    ...
    with col_ga2:
        ...
        fig_aytd.add_trace(go.Scatter(x=df_a_ytd["Afdeling"], y=[None]*len(df_a_ytd), mode='lines', line=dict(color='#00B050', width=4), name='Potensi'))
        ...
        st.plotly_chart(fig_aytd, use_container_width=True, key="chart_yield_pot_afd_ytd")

    col_ta1, col_ta2 = st.columns(2)
    with col_ta1:
        df_ta_mtd = df_a_mtd[['Afdeling', 'Aktual', 'Target']].copy()
        df_ta_mtd.columns = ['Afdeling', 'Aktual', 'Potensi']
        df_ta_mtd['Var'] = df_ta_mtd['Aktual'] - df_ta_mtd['Potensi']
        df_ta_mtd['Pct'] = (df_ta_mtd['Aktual'] / df_ta_mtd['Potensi'] * 100) - 100
        df_ta_mtd.insert(0, 'No', range(1, len(df_ta_mtd) + 1))
        df_ta_mtd.columns = ['No', 'Afdeling', 'Aktual (Ton/Ha)', 'Potensi (Ton/Ha)', 'Gap (Ton/Ha)', 'Var (%)']
        st.dataframe(df_ta_mtd.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Potensi (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_pot_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_yield_pot_afd_mtd")
    
    with col_ta2:
        df_ta_ytd = df_a_ytd[['Afdeling', 'Aktual', 'Target']].copy()
        df_ta_ytd.columns = ['Afdeling', 'Aktual', 'Potensi']
        df_ta_ytd['Var'] = df_ta_ytd['Aktual'] - df_ta_ytd['Potensi']
        df_ta_ytd['Pct'] = (df_ta_ytd['Aktual'] / df_ta_ytd['Potensi'] * 100) - 100
        df_ta_ytd.insert(0, 'No', range(1, len(df_ta_ytd) + 1))
        df_ta_ytd.columns = ['No', 'Afdeling', 'Aktual (Ton/Ha)', 'Potensi (Ton/Ha)', 'Gap (Ton/Ha)', 'Var (%)']
        st.dataframe(df_ta_ytd.style.format({'Aktual (Ton/Ha)': '{:,.2f}', 'Potensi (Ton/Ha)': '{:,.2f}', 'Gap (Ton/Ha)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Ton/Ha)']).map(style_pot_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_yield_pot_afd_ytd")