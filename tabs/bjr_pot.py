...
st.markdown(f"### 🌟 BJR terhadap Potensi (Kg/Jjg)")
...
for col in ['Kg Akt.', 'Jjg Akt.', 'BJR Pot.']:
...
df_k_mtd = df_mtd.groupby('Kebun').apply(
    lambda x: pd.Series({
        ...
        'Target': (x['BJR Pot.'] * x['Jjg Akt.']).sum() / x['Jjg Akt.'].sum() if x['Jjg Akt.'].sum() > 0 else 0
    })
).reset_index()
...
with col_g1:
    ...
    st.plotly_chart(fig_mtd, use_container_width=True, key="chart_bjr_pot_kebun_mtd")
...
with col_g2:
    ...
    st.plotly_chart(fig_ytd, use_container_width=True, key="chart_bjr_pot_kebun_ytd")
...
with col_t1:
    ...
    site_akt_mtd = df_mtd['Kg Akt.'].sum() / tot_jjg_mtd if tot_jjg_mtd > 0 else 0
    site_pot_mtd = (df_mtd['BJR Pot.'] * df_mtd['Jjg Akt.']).sum() / tot_jjg_mtd if tot_jjg_mtd > 0 else 0
    df_total_mtd = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': site_akt_mtd, 'Potensi': site_pot_mtd, 'Var': site_akt_mtd - site_pot_mtd, 'Pct': (site_akt_mtd / site_pot_mtd * 100) - 100 if site_pot_mtd > 0 else 0}])
    
    df_final_mtd = pd.concat([df_t_mtd, df_total_mtd], ignore_index=True)
    df_final_mtd.insert(0, 'No', range(1, len(df_final_mtd) + 1))
    df_final_mtd.columns = ['No', 'Kebun', 'Aktual (Kg)', 'Potensi (Kg)', 'Gap (Kg)', 'Var (%)']
    st.dataframe(df_final_mtd.style.format({'Aktual (Kg)': '{:,.2f}', 'Potensi (Kg)': '{:,.2f}', 'Gap (Kg)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Kg)']).map(style_bjr_var_fill, subset=['Var (%)']).set_properties(subset=['No'], **{'text-align': 'center'}), use_container_width=True, hide_index=True, key="table_bjr_pot_kebun_mtd")
...
with col_t2:
    ...
    site_akt_ytd = df_ytd['Kg Akt.'].sum() / tot_jjg_ytd if tot_jjg_ytd > 0 else 0
    site_pot_ytd = (df_ytd['BJR Pot.'] * df_ytd['Jjg Akt.']).sum() / tot_jjg_ytd if tot_jjg_ytd > 0 else 0
    df_total_ytd = pd.DataFrame([{'Kebun': 'TOTAL SITE', 'Aktual': site_akt_ytd, 'Potensi': site_pot_ytd, 'Var': site_akt_ytd - site_pot_ytd, 'Pct': (site_akt_ytd / site_pot_ytd * 100) - 100 if site_pot_ytd > 0 else 0}])
    
    df_final_ytd = pd.concat([df_t_ytd, df_total_ytd], ignore_index=True)
    df_final_ytd.insert(0, 'No', range(1, len(df_final_ytd) + 1))
    df_final_ytd.columns = ['No', 'Kebun', 'Aktual (Kg)', 'Potensi (Kg)', 'Gap (Kg)', 'Var (%)']
    st.dataframe(df_final_ytd.style.format({'Aktual (Kg)': '{:,.2f}', 'Potensi (Kg)': '{:,.2f}', 'Gap (Kg)': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap (Kg)']).map(style_bjr_var_fill, subset=['Var (%)']).set_properties(subset=['No'], **{'text-align': 'center'}), use_container_width=True, hide_index=True, key="table_bjr_pot_kebun_ytd")

st.markdown("---")
st.markdown("### 🔎 Detail per Afdeling")
list_kebun = sorted(df_raw['Kebun'].dropna().unique())
kebun_terpilih = st.selectbox("Pilih Kebun untuk melihat detail Afdeling:", list_kebun, key="sb_bjr_pot_afd")
df_m_afd = df_mtd[df_mtd['Kebun'] == kebun_terpilih].copy()
df_y_afd = df_ytd[df_ytd['Kebun'] == kebun_terpilih].copy()

if not df_m_afd.empty:
    df_a_mtd = df_m_afd.groupby('Afdeling').agg({'Kg Akt.': 'sum', 'Jjg Akt.': 'sum', 'BJR Pot.': 'mean'}).reset_index()
    df_a_mtd['Aktual'] = df_a_mtd['Kg Akt.'] / df_a_mtd['Jjg Akt.']
    df_a_mtd.rename(columns={'BJR Pot.': 'Target'}, inplace=True)
    df_a_mtd['Pct'] = (df_a_mtd['Aktual'] / df_a_mtd['Target'] * 100).fillna(0)

    df_a_ytd = df_y_afd.groupby('Afdeling').agg({'Kg Akt.': 'sum', 'Jjg Akt.': 'sum', 'BJR Pot.': 'mean'}).reset_index()
    df_a_ytd['Aktual'] = df_a_ytd['Kg Akt.'] / df_a_ytd['Jjg Akt.']
    df_a_ytd.rename(columns={'BJR Pot.': 'Target'}, inplace=True)
    ...
    with col_ga1:
        ...
        st.plotly_chart(fig_amtd, use_container_width=True, key="chart_bjr_pot_afd_mtd")
    ...
    with col_ga2:
        ...
        st.plotly_chart(fig_aytd, use_container_width=True, key="chart_bjr_pot_afd_ytd")

    col_ta1, col_ta2 = st.columns(2)
    with col_ta1:
        df_ta_mtd = df_a_mtd[['Afdeling', 'Aktual', 'Target']].copy()
        df_ta_mtd.columns = ['Afdeling', 'Aktual', 'Potensi']
        df_ta_mtd['Gap'] = df_ta_mtd['Aktual'] - df_ta_mtd['Potensi']
        df_ta_mtd['Var (%)'] = (df_ta_mtd['Aktual'] / df_ta_mtd['Potensi'] * 100) - 100
        df_ta_mtd.insert(0, 'No', range(1, len(df_ta_mtd) + 1))
        st.dataframe(df_ta_mtd.style.format({'Aktual': '{:,.2f}', 'Potensi': '{:,.2f}', 'Gap': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap']).map(style_bjr_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_bjr_pot_afd_mtd")

    with col_ta2:
        df_ta_ytd = df_a_ytd[['Afdeling', 'Aktual', 'Target']].copy()
        df_ta_ytd.columns = ['Afdeling', 'Aktual', 'Potensi']
        df_ta_ytd['Gap'] = df_ta_ytd['Aktual'] - df_ta_ytd['Potensi']
        df_ta_ytd['Var (%)'] = (df_ta_ytd['Aktual'] / df_ta_ytd['Potensi'] * 100) - 100
        df_ta_ytd.insert(0, 'No', range(1, len(df_ta_ytd) + 1))
        st.dataframe(df_ta_ytd.style.format({'Aktual': '{:,.2f}', 'Potensi': '{:,.2f}', 'Gap': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap']).map(style_bjr_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_bjr_pot_afd_ytd")