...
st.markdown(f"### 🌟 RJP terhadap Potensi (Jjg/Pkk)")
...
for col in ['Pokok', 'Jjg Akt.', 'Jjg Pot.']:
...
with col_g1:
    ...
    st.plotly_chart(fig_mtd, use_container_width=True, key="chart_rjp_pot_kebun_mtd")
...
with col_g2:
    ...
    st.plotly_chart(fig_ytd, use_container_width=True, key="chart_rjp_pot_kebun_ytd")
...
with col_t1:
    ...
    tot_pot_mtd = df_mtd['Jjg Pot.'].sum() / tot_pkk_mtd if tot_pkk_mtd > 0 else 0
    df_total_mtd = pd.DataFrame([{
        'Kebun': 'TOTAL SITE', 
        'Aktual': tot_akt_mtd, 
        'Potensi': tot_pot_mtd, 
        'Var': tot_akt_mtd - tot_pot_mtd, 
        'Pct': (tot_akt_mtd / tot_pot_mtd * 100) - 100 if tot_pot_mtd > 0 else 0
    }])
    
    df_final_mtd = pd.concat([df_t_mtd, df_total_mtd], ignore_index=True)
    df_final_mtd.insert(0, 'No', range(1, len(df_final_mtd) + 1))
    df_final_mtd.columns = ['No', 'Kebun', 'Aktual (Jjg/Pkk)', 'Potensi (Jjg/Pkk)', 'Gap', 'Var (%)']
    st.dataframe(df_final_mtd.style.format({'Aktual (Jjg/Pkk)': '{:,.2f}', 'Potensi (Jjg/Pkk)': '{:,.2f}', 'Gap': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap']).map(style_pot_var_fill, subset=['Var (%)']).set_properties(subset=['No'], **{'text-align': 'center'}), use_container_width=True, hide_index=True, key="table_rjp_pot_kebun_mtd")
...
with col_t2:
    ...
    tot_pot_ytd = df_ytd['Jjg Pot.'].sum() / tot_pkk_ytd if tot_pkk_ytd > 0 else 0
    df_total_ytd = pd.DataFrame([{
        'Kebun': 'TOTAL SITE', 
        'Aktual': tot_akt_ytd, 
        'Potensi': tot_pot_ytd, 
        'Var': tot_akt_ytd - tot_pot_ytd, 
        'Pct': (tot_akt_ytd / tot_pot_ytd * 100) - 100 if tot_pot_ytd > 0 else 0
    }])
    
    df_final_ytd = pd.concat([df_t_ytd, df_total_ytd], ignore_index=True)
    df_final_ytd.insert(0, 'No', range(1, len(df_final_ytd) + 1))
    df_final_ytd.columns = ['No', 'Kebun', 'Aktual (Jjg/Pkk)', 'Potensi (Jjg/Pkk)', 'Gap', 'Var (%)']
    st.dataframe(df_final_ytd.style.format({'Aktual (Jjg/Pkk)': '{:,.2f}', 'Potensi (Jjg/Pkk)': '{:,.2f}', 'Gap': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap']).map(style_pot_var_fill, subset=['Var (%)']).set_properties(subset=['No'], **{'text-align': 'center'}), use_container_width=True, hide_index=True, key="table_rjp_pot_kebun_ytd")

# --- AFDELING ---
st.markdown("---")
st.markdown("### 🔎 Detail per Afdeling")
list_kebun = sorted(df_raw['Kebun'].dropna().unique())
kebun_terpilih = st.selectbox("Pilih Kebun untuk melihat detail Afdeling:", list_kebun, key="sb_rjp_pot_afd")
df_m_afd = df_mtd[df_mtd['Kebun'] == kebun_terpilih].copy()
df_y_afd = df_ytd[df_ytd['Kebun'] == kebun_terpilih].copy()

if not df_m_afd.empty:
    df_a_mtd = df_m_afd.groupby('Afdeling').agg({'Jjg Akt.': 'sum', 'Jjg Pot.': 'sum'}).reset_index()
    ...
    df_a_mtd['Target'] = df_a_mtd['Jjg Pot.'] / df_a_mtd['Pokok']
    df_a_mtd['Pct'] = (df_a_mtd['Aktual'] / df_a_mtd['Target'] * 100).fillna(0)

    df_a_ytd = df_y_afd.groupby('Afdeling').agg({'Jjg Akt.': 'sum', 'Jjg Pot.': 'sum'}).reset_index()
    ...
    df_a_ytd['Target'] = df_a_ytd['Jjg Pot.'] / df_a_ytd['Pokok']
    ...
    with col_ga1:
        ...
        st.plotly_chart(fig_amtd, use_container_width=True, key="chart_rjp_pot_afd_mtd")
    ...
    with col_ga2:
        ...
        st.plotly_chart(fig_aytd, use_container_width=True, key="chart_rjp_pot_afd_ytd")
    ...
    with col_ta1:
        ...
        df_ta_mtd.columns = ['No', 'Afdeling', 'Aktual (Jjg/Pkk)', 'Potensi (Jjg/Pkk)', 'Gap', 'Var (%)']
        st.dataframe(df_ta_mtd.style.format({'Aktual (Jjg/Pkk)': '{:,.2f}', 'Potensi (Jjg/Pkk)': '{:,.2f}', 'Gap': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap']).map(style_pot_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_rjp_pot_afd_mtd")
    ...
    with col_ta2:
        ...
        df_ta_ytd.columns = ['No', 'Afdeling', 'Aktual (Jjg/Pkk)', 'Potensi (Jjg/Pkk)', 'Gap', 'Var (%)']
        st.dataframe(df_ta_ytd.style.format({'Aktual (Jjg/Pkk)': '{:,.2f}', 'Potensi (Jjg/Pkk)': '{:,.2f}', 'Gap': '{:+,.2f}', 'Var (%)': '{:+,.1f}%'}).map(style_gap_black, subset=['Gap']).map(style_pot_var_fill, subset=['Var (%)']), use_container_width=True, hide_index=True, key="table_rjp_pot_afd_ytd")