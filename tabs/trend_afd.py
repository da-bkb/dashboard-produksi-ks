...
# --- 4. FILTER TANDEM KEBUN & AFDELING ---
...
df_group['BJR_Tgt'] = df_group['BJR_Tgt'].replace([np.inf, -np.inf], np.nan).fillna(df_group['Bulan'].map(df_filtered.groupby('Bulan', observed=False)[col_tgt_bjr].mean())).fillna(0)
...
st.plotly_chart(fig_yld, use_container_width=True, key="chart_trend_afd_yield")
...
st.plotly_chart(fig_rjp, use_container_width=True, key="chart_trend_afd_rjp")

# ==========================================
# GRAPH 3: TREND BJR (KG/JJG)
# ==========================================
st.markdown("---")
st.subheader(f"⚖️ TREND BJR (KG/JANJANG) - {afd_terpilih}")
fig_bjr = go.Figure()
...
st.plotly_chart(fig_bjr, use_container_width=True, key="chart_trend_afd_bjr")